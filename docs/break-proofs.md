...
### Phase 3, recorded one-off

| Mechanism | Broken by | Test | What it reported |
|---|---|---|---|
| FR-LED-4, the listener acknowledges only after the projection has committed and the dead letters are written (`LedgerEventListener.onBatch`) | a throwaway, never-committed edit moving `acknowledgment.acknowledge()` to the first line of `onBatch` | `LedgerEventConsumerTest#crashBeforeCommitIsRedelivered` | **passed**, so the test does not see this break. Under `AckMode.MANUAL` Spring Kafka commits acknowledged offsets only after the listener returns, and a batch that throws goes to the error handler without its acknowledgements being committed. The placement would matter under `MANUAL_IMMEDIATE`, or if the JVM were killed between an early acknowledge and the commit; the suite simulates neither. What guards it instead: `containerCommitsOnlyAfterTheListener` pins `AckMode.MANUAL` and auto-commit off, the two settings that would make the order load-bearing. Listed under Unproven below as well |

### Phase 3, unproven

| Mechanism | Why there is no break proof | What guards it instead |
|---|---|---|
| The acknowledge call's position after the commit in `LedgerEventListener.onBatch` | see the one-off above: breaking it is invisible under `AckMode.MANUAL`, and making it visible needs a JVM killed mid-batch, which the suite does not do | `LedgerEventConsumerTest.containerCommitsOnlyAfterTheListener` fails if the ack mode or auto-commit changes to one under which the position would decide the outcome |
